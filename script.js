...
function highlightSuggestion(suggestion){
	...
	const highlightedSuggestions = suggestions.getElementsByClassName(highlightedClass);

	[...highlightedSuggestions].forEach(li => {
		if(li !== suggestion){
			li.classList.remove(highlightedClass);
		}
	});
	suggestion.classList.add(highlightedClass);
}
